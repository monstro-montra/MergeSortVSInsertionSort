...
public class Main {
    ...
    static void merge(int[] arr, int[] left, int[] right){
        ...
        // while there are elements in left and right
         while(indexLeft < leftSize && indexRight < rightSize){
            if(left[indexLeft] < right[indexRight]){ //if left array element at indexLeft is < right array element at indexRight
                arr[i] = left[indexLeft]; //set arr element at i = to left array element at indexRight
                i++; //increment i
                indexLeft++; //increment indexLeft
            } else { //if above condition is not true
                arr[i] = right[indexRight]; //set arr element at i = to right array element at indexRight
                i++; //increment i
        ...
        while (indexLeft < left.length){
            arr[i] = left[indexLeft]; //set arr[i] = to element in the left array at indexLeft
            i++; //increment i
            indexLeft++; //increment indexLeft
        }
        while (indexRight < right.length){
            arr[i] = right[indexRight]; //set arr[i] = to element in right array at indexRight
            i++; //increment i
            indexRight++; //increment indexRight
        }
    }
    ...
    static void mergeSort(int[] arr){
        ...
        int j = 0;
        //use for right array
        for(; i < arr.length; i++){ //iterate as long as i is less than arr.length
